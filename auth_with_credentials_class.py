...
class auth_with_apiclient:
    ...
    ** It *should* return an google OAuth2Credentials object.   **
    ...
    class credentials:
        """
        Define class to store user tokens, so the class can be pickled.
        """
        def __init__(self, path=None):
            if not path:
                raise Exception('A path to store the tokens needs to be provided.')
            self.path = path

        def auth(self, auth_code, flow)
